...
struct EmojiArtDocumentView: View {
        ...
        GeometryReader { geometry in
            ...
            ZStack {
                ...
                OptionalImage(uiImage: document.backgroundImage)
                    ...
                    .position(convertFromEmojiCoordinares((0, 0), in: geometry))
    ...
    @State private var backgroundPicker: BackgroundPickerStyle?
    
    enum BackgroundPickerStyle: Identifiable {
        case camera
        case library
        var id: BackgroundPickerStyle { self }
    }
        ...
        if !found {
            found = providers.loadObjects(ofType: String.self) { string in
                if let emoji = string.first, emoji.isEmoji {
                    document.addEmoji(
                        String(emoji),
                        at: convertToEmojiCoordinares(location, in: geometry),
    ...
    private func position(for emoji: EmojiArtModel.Emoji, in geometry: GeometryProxy) -> CGPoint {
        convertFromEmojiCoordinares((emoji.x, emoji.y), in: geometry)
    }
